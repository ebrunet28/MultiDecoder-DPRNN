...
def main(args):
    ...
    # model
    base = ConvTasNet(N = 256, L = 20, B = 256, H = 512, P = 3, X = 8, R = 2, I = 1, C = 3, norm_type="gLN", causal=0,
                mask_nonlinear='relu')
    blackbox = ConvTasNet(N = 256, L = 20, B = 256, H = 512, P = 3, X = 8, R = 2, I = 4, C = 3, norm_type="gLN", causal=0,
                mask_nonlinear='relu')
    if args.use_cuda:
        base = torch.nn.DataParallel(base)
        base.cuda()
        blackbox = torch.nn.DataParallel(blackbox)
        blackbox.cuda()
    # optimizer
